...
from dataloaders.acdc import ACDC_DIFF_FSP
from networks.net_factory import net_factory
from utils import losses, val_2d


parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='/home/wth/My_codes/ACDC', help='Name of Experiment')
parser.add_argument('--dataset', type=str, default = 'ACDC', help='the dataset used in the experiments')

parser.add_argument('--labeled-id-path', type=str, default='/home/wth/My_codes/ACDC/7_labeled_1.txt')
parser.add_argument('--unlabeled-id-path', type=str, default='/home/wth/My_codes/ACDC/7_unlabeled_1.txt')

parser.add_argument('--exp', type=str, default=r'fsp_new_time1_RE1', help='experiment_name')
...
def self_train(args, snapshot_path):
    ...
    trainset_u = ACDC_DIFF_FSP(args.dataset, args.root_path, 'train_u', 
                             args.patch_size[0], args.unlabeled_id_path)
    trainset_l = ACDC_DIFF_FSP(args.dataset, args.root_path, 'train_l', 
                             args.patch_size[0],args.labeled_id_path, nsample=len(trainset_u.ids))
    
    db_val = ACDC_DIFF_FSP(args.dataset, args.root_path, "val")
    ...
    for _ in iterator:
        ...
        for i,((img_x, mask_x),
            ...
            #loss = (loss_sup + 0.25 * loss_unsup + 0.25 * loss_con + 0.5 * loss_fp) / 2.0
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for _, (img, mask) in enumerate(valloader):
                    metric_i = val_2d.test_single_volume(img, mask, model, classes=num_classes)
...
if __name__ == "__main__":
    ...
    self_snapshot_path = "/home/wth/My_codes/mia_ssl_code/model/{}_{}_{}_labeled/".format(args.dataset,args.exp, args.labelnum)
